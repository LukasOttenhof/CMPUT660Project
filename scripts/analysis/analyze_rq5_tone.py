...
from data_loader import load_all  # same as in analyze_rq5
...
TABLES_DIR = ROOT / "outputs" / "rq5" / "tables"
PLOTS_DIR = ROOT / "outputs" / "rq5" / "plots"
...
def run() -> None:
    ...
    for name, (before_key, after_key, text_col, time_col) in TEXT_TARGETS.items():
        ...
        combined_path = TABLES_DIR / f"rq5_tone_{name}.csv"
        combined.to_csv(combined_path, index=False)
        print(f"[rq5-tone] Saved per-doc sentiment → {combined_path}")
        ...
        summary_path = TABLES_DIR / f"rq5_tone_summary_{name}.csv"
        summary.to_csv(summary_path, index=False)
        print(f"[rq5-tone] Saved tone summary → {summary_path}")
        ...
        plt.tight_layout()
        plt.savefig(PLOTS_DIR / f"rq5_tone_{name}.png", dpi=300)
        plt.close()


    print("[rq5-tone] Sentiment analysis complete.")
